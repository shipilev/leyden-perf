...
# Custom build
JT=/home/shade/trunks/shipilev-leyden/build/linux-x86_64-server-release/images/jdk/
OUT=results/training-impact/leyden-new
...
TRAIN_ITERS="1 2 4 8 16 32 64 128 256 512 1024 2048 4096 8192 16384 32768"
#RUN_ITERS="1 2 4"
#ITERS="1 2 4 8 16 32 64 128 256 512 1024"
#RUN_ITERS="1 2 4 8 16 32 64 128"
RUN_ITERS="1 2 4 8 16 32 64 128 256 512 1024 2048"
#ITERS="1 2 4 8 16"
#ITERS="1024"
...
OPTS="-XX:+UseSerialGC -Xms64m -Xmx3g -cp JavacBenchApp.jar JavacBenchApp"

if [ "x" == "x${GRAPH_ONLY}" ]; then
	rm -f *.aot *.aotconf
	for TI in $TRAIN_ITERS; do
...
for RI in $RUN_ITERS; do
cat <<EOF >> plot.gnu
set title "javac: compile $RI classes"
plot "./base-data-$RI.dat" using 1:(\$2*1000) lw 5 with lines title 'no AOT', \
     "./data-$RI.dat" using 1:(\$2*1000):(\$2-\$3)*1000:(\$2+\$3)*1000 lw 5 with errorbars title 'AOT'
EOF
done
